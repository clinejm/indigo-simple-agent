...
function get_blinding_integer(num_bytes=4) {
  r_buff = Buffer.alloc(num_bytes)
  rng_get_bytes(r_buff)
  return parseInt(r_buff.toString('hex'), 16)
}

function get_next_evaluator(evaluators) {
  if (evaluators.round_one.length > 0)
    return evaluators.round_one[0];
  if (evaluators.round_two.length > 0)
    return evaluators.round_two[0];
  return evaluators.requested_by;
}
