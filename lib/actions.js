...
module.exports = {
  ...
  init: function(object_description, pk_str) {
    ...
    this.state = {
      ...
      total: r,
      evaluators: {
        round_one: evaluators,
        round_two: evaluators,
        requested_by: {
          e: User.rsa_key.e, 
          n: User.rsa_key.n
        }
  ...
  evaluate: function(value) {
    ...
    if (this.state.evaluators.round_one.length==0) {
      return this.reject('all valuations have been given!');
    }
    ...
    // im no longer necessary
    this.state.evaluators.round_one.shift();
  ...
  unblind: function(r) {
    ...
    if (this.state.evaluators.round_two.length==0) {
      return this.reject('all unblinds have been given!');
    }
    ...
    // im no longer necessary
    this.state.evaluators.round_two.shift();
