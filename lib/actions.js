...
user_name = process.argv[3] || "user"
const User = require('../config/'+user_name);

console.log("my key: ", User.rsa_key)
...
function encryptTotal(state, total){
  ...
  //console.log("next",next, next_status.index);
  var encrypted_total = encrypt(total.toString(), next.n, next.e);
...
module.exports = {
  ...
  evaluate: function(value) {
    ...
    if (!checkStatusIs(state, "valuation")) {
      return this.reject('All values have been provided. Current status:' + state.status.round);
    }
  ...
  unblind: function() {
    ...
    if (!checkStatusIs(state, "unblinding")) {
      return this.reject("Can't unblind now. Current status:" + state.status.round);
    }
  ...
  finalize: function() {
    ...
    if (!checkStatusIs(state, "finalize")) {
      return this.reject("NO!! Current status:" + state.status.round);
    }
